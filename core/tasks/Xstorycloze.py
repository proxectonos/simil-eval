...
class Xstorycloze(SimilarityTask):
    ...
    def __init__(self, lang, cache, token):
        ...
        # Define the field mappings for different languages
        self.field_mappings = {
            "gl": {
    ...
    def load_data(self):
        ...
        if hf_dataset=="None":
            ...
            self.dataset = load_dataset(repo, subset, cache_dir = self.cache)["eval"][0:500]
        else:
            if self.lang == "cat":
                self.dataset = load_dataset(hf_dataset, cache_dir = self.cache)["eval"][0:500]
            else: #gl, pt  name "test" instead of "eval"
                self.dataset = load_dataset(hf_dataset, cache_dir = self.cache)["test"][0:500]
